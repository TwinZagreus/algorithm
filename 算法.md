# 算法
## 1基础思想
### 1.1数据结构的存储方式只有两种：数组（顺序存储）和链表（链式存储）
...
var search = function(nums, target) {
    ...
    while(left<=right){//左闭右闭right包含在内
...
var search = function(nums, target) {
    ...
    while(left<right){//左闭右开right不包含在内
    	let mid=Math.floor((left+right)/2);
        if(nums[mid]>target){
...
var removeElement = function(nums, val) {
    ...
    for(let fastIndex=0;fastIndex<len;fastIndex++){
        if(nums[fastIndex]!=val){//只有快指针不等于要删除的值的时候慢指针才增加且赋值
...
数组向右移一位（用-1做首位，遇到冲突向本索引的值获取新的索引）
...
var repeatedSubstringPattern = function(s) {
    ...
    return tb[tb.length-1]!=0&&s.length%(s.length-tb[tb.length-1])===0?true:false
};
```
