...
1、复用逻辑困难，render-props以及HOC高阶组件

2、复杂组件难以理解，需要在生命周期中处理多逻辑及状态
...
函数组件没有类组件的生命周期函数， componentDidMount(一般用于发请求)，componentDidUpdate(用于更新)，componentWillUnmount(卸载)

useEffect可以让函数组件进行副作用，相当于componentDidMount(一般用于发请求)，componentDidUpdate(用于更新)，componentWillUnmount(卸载)的组合

useEffect(() => {}) : 回调函数会在组件render后执行，，，，，页面加载会执行一次，只要有任何数据发生了改变，组件就会更新，，，useEffect这个函数就要执行。

 useEffect(() => {}, [count]); 回调函数会在加载的时候执行一次， 在每次组件更新的时候，会判断count是否有变化， 如果没有变化，useEffect就不执行。。。。。只会在count变化的时候执行useEffect

 useEffect(() =>{  }, []), 只会在组件加载的时候执行一次，后续就不在执行了，，，，因为空数组永远不会变化。

 useEffect(() => {  return function cleanup() {清理} }) cleanup会在每次执行useEffect之前执行，包括组件销毁的时候也要执行，，，，cleanup是可以选。 需要清除副作用，才会使用
...
父组件的数据，在子组件上添加需要传递的数据属性名，子组件通过this.props.数据属性名，接收父组件传递过来的数据进行渲染

### 子组件给父组件传值

在父组件中声明一个函数,父组件通过props把函数给子组件(传给子组件函数需要处理this问题，bind(this)或者箭头函数)

函数中接收需要的参数，并且在调用子组件的标签上添加该函数

子组件通过props接收到该函数，进行调用，并且把自己的数据传给父组件
...
使用const { Provider, Consumer } = React.createContext()

Provider是提供数据,

Provider标签包裹整个组件，在其标签上添加value属性即是需要传递的数据

Consumer是消费数据

Consumer也包裹需要传递的那个组件，但是需要使用一个函数返回一段结构并渲染

## 说说你对Redux的理解？其工作原理？

三大核心：action Creactor ,store,reducer

store相当于老板，action Creactor提出想法和操作，reducer去执行

store将state和action交给reducer，reducer执行完毕返回一个新state
...
action Creactor其实是一个函数，用来创建action

reducer也是一个函数，有两个参数，参数1：state，参数2：action ，最终返回新state，reducer不会修改原数据

reducer这个函数必须是纯函数，也就是同样是输入，总会得到同样的输出，不能出现不纯的方法

在reducer函数内部先判断action的type值再进行逻辑操作
